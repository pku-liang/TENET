...
void DataflowAnalysis(shared_ptr<ISL_Context> context, const char* _pe_file, const char* _statement_file,
                      const char* _mapping_file)
{
    ...
    for (auto& iter : output) {
        double reuse_factor   = df.GetReuseFactor(iter, AccessType::WRITE, isl_union_map_copy(space_time_to_neighbor));
...
int experiment(shared_ptr<ISL_Context> context, path experiment_file)
{
    ...
    DataflowAnalysis(context, (prefix + pe_array).c_str(), (prefix + statement).c_str(), (prefix + mapping).c_str());
