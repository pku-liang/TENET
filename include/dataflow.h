...
public:
	...
	double GetSpatialReuseVolume(string tensor_name, AccessType type, isl_union_map* stt_neighbor);
	int GetMacNum(int mac_per_instance = 1);
	int GetMacNumPerPE(int mac_per_instance = 1);
	int GetActivePENum();
	double GetAverageActivePENum();
	int GetIngressDelay(isl_union_map* space_time_to_neighbor ,string tensor_name = "");
	int GetEgressDelay(isl_union_map* space_time_to_neighbor, string tensor_name = "");
